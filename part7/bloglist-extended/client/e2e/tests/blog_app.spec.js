...
describe('Blog app', () => {
  ...
  describe('Login', () => {
    test('succeeds with correct credentials', async ({ page }) => {
      ...
      await expect(
        page.getByText('Matti Luukkainen logged in').first()
      ).toBeVisible()
    })
  ...
  describe('When logged in', () => {
    ...
    test('remove button is not shown to a user who did not create the blog', async ({
      page,
    }) => {
      await createBlog(
        page,
    ...
    test('blogs are ordered according to likes, most liked first', async ({
      page,
    }) => {
